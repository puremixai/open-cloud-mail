<template>
  <div v-if="uiStore.win95" ref="desktopRef" class="w95-desktop" @click="desktopClick" @contextmenu="onDesktopContextmenu">
    ...
    <button type="button" class="w95-dicon" @click.stop="openIdcard">
      <svg width="32" height="32" viewBox="0 0 32 32">
...
import IdentityCard from './idcard.vue'
...
function closeIdcard() {
  ...
  nextTick(() => desktopRef.value?.querySelectorAll('.w95-dicon')[2]?.focus())
}
...
const windowTitle = computed(() => {
  ...
  const title = settingStore.siteTitle
...
function showAbout() {
  ElMessageBox.alert(
      `${settingStore.siteTitle} — ${t('win95AboutBody')}`,
      t('about'),
      { confirmButtonText: t('confirm') }
  )
}
